...
namespace best
{
    public partial class stafftruck : Form
    {
        ...
        private void loadTruck()
        {
            ...
            con.Open();
            cmd = new MySqlCommand("SELECT ID, plateNum ,yearModel,category,status FROM truck;", con);
        ...
        private void addTrucks()
        {
            ...
            cmd = new MySqlCommand("INSERT INTO " +
                "truck (plateNum, yearModel, category,status) " +
                "VALUES (@pnum, @ym, @category, @stat);", con);
            ...
            cmd = new MySqlCommand("INSERT INTO ltorenewal (PlateNo,Duedate) " +
                                   "VALUES (@PlateNo, @Duedate);", con);
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (data_trucks.SelectedRows.Count > 0)
            {
                ...
                // Update truck table
                cmd = new MySqlCommand("UPDATE truck SET plateNum = @plateNum, yearModel = @yearModel, category = @category, status = @status WHERE ID = @id;", con);
                cmd.Parameters.AddWithValue("@plateNum", txtPlateNum.Text);
                ...
                cmd = new MySqlCommand("UPDATE ltorenewal SET PlateNo = @plateNum, Duedate = @Duedate WHERE PlateNo = @oldPlateNum;", con);
                cmd.Parameters.AddWithValue("@plateNum", txtPlateNum.Text);
                cmd.Parameters.AddWithValue("@Duedate", dueDate.ToString("yyyy-MM-dd"));
                cmd.Parameters.AddWithValue("@oldPlateNum", oldPlateNum);
                cmd.ExecuteNonQuery();
                ...
                // Update status in delivery table if needed
        ...
        private void data_trucks_SelectionChanged(object sender, EventArgs e)
        {
            if (data_trucks.SelectedRows.Count > 0)
            {
                ...
                cbStatus.Text = data_trucks.SelectedRows[0].Cells[4].Value.ToString();
            }
        }
